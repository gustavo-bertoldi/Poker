public class Intelligence{
    ...
    private static int intelligenceNiveau1 (Jeu jeu, Joueur joueur){
        ...
        if (!jeu.flop) { //quand le joueur a seulement ses 2 cartes à lui
           ...
           if (j.getHand().getValeurHandSurMain() > 14) { //si le joueur a au moins une paire, il va doubler le pari
                decision =  2 * jeu.pariActuel;
           }

           else if (j.bigBlind) { //si le joueur est le Big Blind, il va toujours passer au moment 1
               decision = 1;
           }

           else if (jeu.pariActuel < (1 / 6) * j.getArgent()) { //si le pari actuel est peu important en comparaison à l´argent du joueur, le joueur paye toujours le pari
                decision = 1;

           }

           else { //si le pari actuel est assez important
                if (j.getHand().getValeurHandSurMain() >= 8) { //si le joueur a une carte supérieure ou égale à 8 (et pas de paires ni rien meilleur), il paye le parie. Sinon, il arrête de jouer.
                    decision = 1;
                } else {
                    decision = 0;
                }
            ...
            if (j.getHand().getValeurHandApresFlop() < 8 && jeu.pariActuel > 0) { //si le joueur était le Big Blind mais il n´a que des cartes inférieures à 8 (et pas de paires ni rien meilleur), il arrête de jouer
                decision = 0;
            }

           else if (j.getHand().getValeurHandApresFlop() < 60) { //si le joueur a une paire de 5 ou une Hand moins importante, il paye le pari actuel. Si il a une Hand plus importante, il augmente de 100 le pari actuel.
                decision = 1;
            } else {
				if(j.derniereValeurPariee < 100){
                decision = jeu.pariActuel + 100;
			} else {
				decision = 1;
			}
            }
            ...
            else if (j.getHand().getValeurHandApresTurn() == j.getHand().getValeurHandApresFlop()) {  //si le joueur n´a pas une meilleure Hand qu´au moment précédent, le joueur paye le pari actuel
                decision = 1;

            } else {
				if(j.derniereValeurPariee < 100){
                decision = (jeu.pariActuel + 70); //sinon (sa valeur de Hand a augmenté), il augmente de 70 le pari actuel
			} else { 
				decision = 1;
			}
		}
            ...
            else if (j.getHand().getValeurHandApresRiver() == j.getHand().getValeurHandApresTurn()) { //si le joueur n´a pas une meilleure Hand qu´au moment précédent, le joueur paye le pari actuel
                decision = 1;
            } else {
				if (j.derniereValeurPariee < 100){
                decision =  jeu.pariActuel + 50; //sinon (sa valeur de Hand a augmenté), il augmente de 50 le pari actuel
			} else{
				decision = 1;
			}
            }
