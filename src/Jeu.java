...
public class Jeu extends Thread {
    ...
    protected int nJoueursQuiOntPayeLeTour; //Utilisé dans le calcul des pots secondaires
    ...
    protected Joueur dernierAParier; //Défini au début d'une tournée, c'est le dernier joueur qui va jouer, après lui la tournée est finie
    ...
    public Jeu(String nomJoueurHumain) throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            joueurs.add(new Ordinateur(nomsJoueursOrdinateurs.get(a), 1));
        ...
        ((Ordinateur) joueurActuel.joueur).jouer(pariActuel, this); // À l'initialisation d'un jeu, ce n'est jamais l'humain qui commence
   ...
   */
    ...
    public void prochaineTournee() throws Exception {
        ...
        joueurs.getJoueurs().forEach(joueur -> {
            ...
            joueur.prochaineTournee(moment);
        });
        ...
        if (joueursDansLaTournee.size() >= 4) {
            ...
            ancienDealer.prochainNode.joueur.setRoleJeu(true, false, true, false);
            ancienDealer.joueur.setRoleJeu(false, true, false, true);
        }
    ...
    public void prochainJoueur() throws Exception {
        System.out.println("Joueur: " + joueurActuel.joueur.nom +" appelle prochain Joueur");
        joueurActuel = joueurActuel.prochainNode;
        System.out.println("Joueur Actuel devient: " + joueurActuel.joueur.nom );
        // creer methode pour finir tournee -> sur joueur, il doit savoir s'il es le dernier a parier ou pas
        if (joueursDansLaTournee.size() == 1) {
            ...
            if(joueurActuel.joueur.allIn){
                joueurActuel.joueur.setActionJoueurHumain(0,pariActuel,this); // juste passer au prochain joueur
            }
            else{
    ...
    /*
    Relancer le paris si un joueur a parie
     */
    public void relancerParis(Joueur joueurAParie) throws Exception{
        joueursDansLaTournee.getJoueurs().forEach(joueur -> {
                    joueur.dernierAParier = false;
                });
        dernierAParier = (joueursDansLaTournee.getNodeAnterieur(joueurAParie)).joueur;
        while(!dernierAParier.dansTourneeParis){
            dernierAParier = joueursDansLaTournee.getNodeAnterieur(dernierAParier).joueur;
        }
    ...
    On vérifie d'abord si tous les joueurs on mis All In: Si c'est le cas la tournée finit et on cherche le gagnant
    ...
    public void tourDeParisFini() throws Exception {
        ...
        //Sinon on poursuit le jeu
            ...
            if (moment == 0) {
                ...
                tourneeFinie();
                System.out.println("Tour de paris fini + Tournee Finie");
            }
        }
    }
    ...
    public void tourneeFinie() throws Exception {
        ...
        //Cas où tous les tours de paris sont finis, on cherche le(s) gagnant(s).
        else {
    ...
    public void creerPotsSecondaires(Joueur j){
        ...
        if(!flop){
            potsSecondaires.put(j, (j.getArgent()+j.derniereValeurPariee)*(nJoueursQuiOntPayeLeTour+1));
        }
        else if(!turn){
            potsSecondaires.put(j, potAvantFlop + (j.getArgent()+j.derniereValeurPariee)*(nJoueursQuiOntPayeLeTour+1));
        }
        else if(!river){
            potsSecondaires.put(j, potAvantTurn + (j.getArgent()+j.derniereValeurPariee)*(nJoueursQuiOntPayeLeTour+1));
        }
        else{
            potsSecondaires.put(j, potAvantRiver + (j.getArgent()+j.derniereValeurPariee)*(nJoueursQuiOntPayeLeTour+1));
        }
        System.out.println(potsSecondaires.toString());
    }
    ...
    */
    ...
    public LinkedList<Joueur> trouverJoueursGagnants(LinkedList<Joueur> joueursDansLaTournee){
        ...
        for(Joueur j : joueursDansLaTournee){
            j.getHand().setHandMoment(3);
        }
